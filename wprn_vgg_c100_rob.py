...
class FilterStatsCollector:  
    ...
    def get_rob_saliency(self, r_scale=0.05, d_scale=0.05):
        fisher_scores = {}
        ...
        for class_label in self.class_means.keys():
            m_c = self.class_means[class_label].detach().numpy()
            m_cp = self.complement_means[class_label].detach().numpy()
            S_c = self.class_variances[class_label].detach().numpy()
            S_cp = self.complement_variances[class_label].detach().numpy()

            T_scores = np.zeros(num_filters)
            ...
            for j in range(num_filters):
                ...
                # Constraints
                r = r_scale * (np.linalg.norm(S_c[j], 'fro') + np.linalg.norm(S_cp[j], 'fro'))
                d1 = d_scale * np.linalg.norm(m_c[j])
                d2 = d_scale * np.linalg.norm(m_cp[j])
                constraints = [cp.norm(m1 - m_c[j]) <= d1, cp.norm(m2 - m_cp[j]) <= d2]

                # Objective
                S = S_c[j] + S_cp[j] + r * np.eye(2)
                objective = cp.Minimize(cp.quad_form(m1 - m2, np.linalg.inv(S)))
                ...
                if prob.status not in ["infeasible", "unbounded"]:
                    ...
                    print(f"Problem not solved for filter {j}. Status: {prob.status}")

            fisher_scores[class_label] = T_scores

        return fisher_scores, torch.from_numpy(saliency_scores)
...
if __name__ == '__main__':
    ...
    for k in range(len(conv_dict)):
        ...
        rob_scores, saliency = collector.get_fish_saliency()
