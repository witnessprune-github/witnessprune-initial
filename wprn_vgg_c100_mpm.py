    ...
    def get_mpm_saliency(self):
        ...
        for class_label in self.class_means.keys():
            ...
            for j in range(num_filters):
                ...
                # Constraints
                constraints = [u.T @ (m_c[j] - m_cp[j]) == 1]

                # Objective
                S1_half = np.linalg.matrix_power(S_c[j], 0.5)
                S2_half = np.linalg.matrix_power(S_cp[j], 0.5)
                objective = cp.Minimize(cp.norm(S1_half @ u) + cp.norm(S2_half @ u))
                ...
                if prob.status not in ["infeasible", "unbounded"]:
                    # Calculating Z and F_c
                    Z = np.linalg.norm(S1_half @ u.value) + np.linalg.norm(S2_half @ u.value)
                    F_c = 1 / Z
                    T_c = (F_c / (np.sqrt(2) + F_c)) ** 2
                    T_scores[j] = T_c
...
if __name__ == '__main__':
    ...
    for k in range(len(conv_dict)):
        ...
        mpm_scores, saliency = collector.get_mpm_saliency()
